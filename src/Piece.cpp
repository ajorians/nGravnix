...
#include "GravnixLevels.h"//For the piece type defines

PieceControl::PieceControl(SDL_Surface* pScreen, Metrics* pBoardMetrics, Config* pConfig, MovesRemainingLabel* pLabel)
: m_pScreen(pScreen), m_pBoardMetrics(pBoardMetrics), m_pConfig(pConfig), m_pMovesLabel(pLabel), m_pRoot(NULL)
{
}
...
bool PieceControl::IsAnimating() const
{
   ...
   for(pCurrent = m_pRoot; pCurrent != NULL; pCurrent = pCurrent->next) {
      ...
      if( IsPieceFading(pCurrent, m_pConfig->GetFadeSteps()) )
         return true;
      if( IsPieceShrinking(pCurrent, m_pConfig->GetFadeSteps()) )//maybe new config item
         return true;
   }
...
bool PieceControl::Animate()
{
   ...
   while(pCurrent != NULL) {
      ...
      ShrinkPieceSprite(m_pScreen, pCurrent, m_pConfig->GetFadeSteps());
   ...
   while(pCurrent != NULL) {
      ...
      pCurrent = pCurrent->next;
      if( (pTemp->toFade == true || pTemp->toShrink) && pTemp->nFadeAmount >= m_pConfig->GetFadeSteps() ) {
